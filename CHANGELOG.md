# Changelog
All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [0.16.3](https://github.com/robo9k/rust-magic/compare/v0.16.2...v0.16.3) - 2024-06-07

### Other
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.60 to 0.5.61
- *(deps)* Bump taiki-e/install-action from 2.33.35 to 2.36.0
- *(deps)* Bump github/codeql-action from 3.25.6 to 3.25.8
- *(deps)* Bump actions/create-github-app-token from 1.10.0 to 1.10.1
- *(deps)* Bump taiki-e/install-action from 2.33.32 to 2.33.35
- *(deps)* Bump actions/create-github-app-token from 1.9.3 to 1.10.0
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.58 to 0.5.60
- *(deps)* Bump thiserror from 1.0.60 to 1.0.61
- *(deps)* Bump libc from 0.2.154 to 0.2.155
- *(deps)* Bump ossf/scorecard-action from 2.3.1 to 2.3.3
- *(deps)* Bump github/codeql-action from 3.25.5 to 3.25.6
- *(deps)* Bump codecov/codecov-action from 4.4.0 to 4.4.1
- *(deps)* Bump step-security/harden-runner from 2.7.1 to 2.8.0
- *(deps)* Bump taiki-e/install-action from 2.33.20 to 2.33.32
- *(deps)* Bump cargo-careful from 0.4.0 to 0.4.2
- *(deps)* Bump rui314/setup-mold
- *(deps)* Bump codecov/codecov-action from 4.3.0 to 4.4.0
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.56 to 0.5.58
- *(deps)* Bump github/codeql-action from 3.25.4 to 3.25.5
- *(deps)* Bump actions/checkout from 4.1.5 to 4.1.6
- *(deps)* Bump step-security/harden-runner from 2.7.0 to 2.7.1
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.51 to 0.5.56
- *(deps)* Bump taiki-e/install-action from 2.33.13 to 2.33.20
- *(deps)* Bump github/codeql-action from 3.25.3 to 3.25.4
- *(deps)* Bump actions/checkout from 4.1.4 to 4.1.5
- *(deps)* Bump thiserror from 1.0.59 to 1.0.60
- *(deps)* Bump github/codeql-action from 3.25.1 to 3.25.3
- *(deps)* Bump actions/upload-artifact from 4.3.2 to 4.3.3
- *(deps)* Bump actions/checkout from 4.1.2 to 4.1.4
- *(deps)* Bump dtolnay/rust-toolchain
- *(deps)* Bump taiki-e/install-action from 2.32.20 to 2.33.13
- *(deps)* Bump libc from 0.2.153 to 0.2.154
- *(deps)* Bump thiserror from 1.0.58 to 1.0.59
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.50 to 0.5.51
- *(deps)* Bump codecov/codecov-action from 4.2.0 to 4.3.0
- *(deps)* Bump actions/upload-artifact from 4.3.1 to 4.3.2
- *(deps)* Bump github/codeql-action from 3.24.10 to 3.25.1
- *(deps)* Bump taiki-e/install-action from 2.32.14 to 2.32.20
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.49 to 0.5.50
- *(deps)* Bump github/codeql-action from 3.24.9 to 3.24.10
- *(deps)* Bump dtolnay/rust-toolchain
- *(deps)* Bump actions/create-github-app-token from 1.9.1 to 1.9.3
- *(deps)* Bump taiki-e/install-action from 2.32.7 to 2.32.14
- *(deps)* Bump actions/configure-pages from 4.0.0 to 5.0.0
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.48 to 0.5.49
- *(deps)* Bump codecov/codecov-action from 4.1.1 to 4.2.0
- *(deps)* Bump dtolnay/rust-toolchain
- *(deps)* Bump taiki-e/install-action from 2.31.2 to 2.32.7
- *(deps)* Bump bitflags from 2.4.2 to 2.5.0
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.47 to 0.5.48
- *(deps)* Bump rui314/setup-mold
- *(deps)* Bump taiki-e/install-action from 2.29.4 to 2.31.2
- *(deps)* Bump actions/create-github-app-token from 1.9.0 to 1.9.1
- *(deps)* Bump codecov/codecov-action from 4.1.0 to 4.1.1
- *(deps)* Bump actions/deploy-pages from 4.0.4 to 4.0.5
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.46 to 0.5.47
- *(deps)* Bump taiki-e/install-action from 2.28.14 to 2.29.4
- *(deps)* Bump github/codeql-action from 3.24.7 to 3.24.9
- *(deps)* Bump cargo-deny from 0.14.3 to 0.14.19
- *(deps)* Bump thiserror from 1.0.57 to 1.0.58
- *(deps)* Bump actions/create-github-app-token from 1.8.1 to 1.9.0
- *(deps)* Bump rui314/setup-mold
- *(deps)* Bump actions/checkout from 4.1.1 to 4.1.2
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.41 to 0.5.46
- *(deps)* Bump github/codeql-action from 3.24.5 to 3.24.7
- *(deps)* Bump taiki-e/install-action from 2.27.6 to 2.28.14
- *(deps)* Bump fsfe/reuse-action from 2.0.0 to 3.0.0
- *(deps)* Bump codecov/codecov-action from 4.0.1 to 4.1.0
- *(deps)* Bump actions/upload-pages-artifact from 3.0.0 to 3.0.1
- *(deps)* Bump taiki-e/install-action from 2.27.1 to 2.27.6
- *(deps)* Bump github/codeql-action from 3.24.4 to 3.24.5
- *(deps)* Bump step-security/harden-runner from 2.6.1 to 2.7.0
- *(deps)* Bump codecov/codecov-action from 3.1.4 to 4.0.1
- *(deps)* Bump actions/create-github-app-token from 1.6.3 to 1.8.1
- *(deps)* Bump rui314/setup-mold
- *(deps)* Bump actions/deploy-pages from 4.0.3 to 4.0.4
- *(deps)* Bump github/codeql-action from 3.24.3 to 3.24.4
- *(deps)* Bump thiserror from 1.0.56 to 1.0.57
- *(deps)* Bump libc from 0.2.152 to 0.2.153
- *(deps)* Bump bitflags from 2.4.1 to 2.4.2
- *(deps)* Bump taiki-e/install-action from 2.24.3 to 2.27.1
- *(deps)* Bump github/codeql-action from 3.23.0 to 3.24.3
- *(deps)* Bump actions/upload-artifact from 4.0.0 to 4.3.1
- *(deps)* Bump Swatinem/rust-cache from 2.7.2 to 2.7.3
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.34 to 0.5.41
- *(deps)* Bump actions/deploy-pages from 4.0.2 to 4.0.3
- *(deps)* Bump github/codeql-action from 3.22.12 to 3.23.0
- *(deps)* Bump actions/create-github-app-token from 1.6.2 to 1.6.3
- *(deps)* Bump taiki-e/install-action from 2.23.8 to 2.24.3
- *(deps)* Bump Swatinem/rust-cache from 2.7.1 to 2.7.2
- *(deps)* Bump libc from 0.2.151 to 0.2.152
- *(deps)* Bump thiserror from 1.0.52 to 1.0.56
- *(deps)* Bump rui314/setup-mold
- *(deps)* Bump taiki-e/install-action from 2.23.6 to 2.23.8
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.31 to 0.5.34
- *(deps)* Bump dtolnay/rust-toolchain
- *(deps)* Bump taiki-e/install-action from 2.22.2 to 2.23.6
- *(deps)* Bump actions/deploy-pages from 3.0.1 to 4.0.2
- *(deps)* Bump actions/upload-pages-artifact from 2.0.0 to 3.0.0
- *(deps)* Bump github/codeql-action from 3.22.11 to 3.22.12
- *(deps)* Bump thiserror from 1.0.50 to 1.0.52
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.29 to 0.5.31
- *(deps)* Bump actions/upload-artifact from 3.1.3 to 4.0.0
- *(deps)* Bump github/codeql-action from 2.22.9 to 3.22.11
- *(deps)* Bump actions/deploy-pages from 2.0.4 to 3.0.1
- *(deps)* Bump taiki-e/install-action from 2.21.27 to 2.22.2
- *(deps)* Bump libc from 0.2.150 to 0.2.151
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.27 to 0.5.29
- *(deps)* Bump actions/create-github-app-token from 1.6.0 to 1.6.2
- *(deps)* Bump actions/configure-pages from 3.0.6 to 4.0.0
- *(deps)* Bump github/codeql-action from 2.22.8 to 2.22.9
- *(deps)* Bump taiki-e/install-action from 2.21.24 to 2.21.27
- *(deps)* Bump taiki-e/install-action from 2.21.18 to 2.21.24
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.26 to 0.5.27
- *(deps)* Bump actions/create-github-app-token from 1.5.1 to 1.6.0
- Update deprecated parameters for `create-github-app-token` action
- *(deps)* Bump github/codeql-action from 2.22.5 to 2.22.8
- *(deps)* Bump taiki-e/install-action from 2.21.10 to 2.21.18
- *(deps)* Bump step-security/harden-runner from 2.6.0 to 2.6.1
- *(deps)* Bump libc from 0.2.149 to 0.2.150
- *(deps)* Bump taiki-e/install-action from 2.21.7 to 2.21.10
- *(deps)* Bump MarcoIeni/release-plz-action from 0.5.25 to 0.5.26
- *(deps)* Bump actions/create-github-app-token from 1.5.0 to 1.5.1
- *(deps)* Bump github/codeql-action from 2.22.4 to 2.22.5
- *(deps)* Bump taiki-e/install-action from 2.20.17 to 2.21.7
- *(deps)* Bump ossf/scorecard-action from 2.3.0 to 2.3.1
- *(deps)* Bump Swatinem/rust-cache from 2.7.0 to 2.7.1
- *(deps)* Bump taiki-e/install-action from 2.20.11 to 2.20.17
- *(deps)* Bump github/codeql-action from 2.22.3 to 2.22.4
- *(deps)* Bump taiki-e/install-action from 2.20.10 to 2.20.11
- Use regular `release-plz-action` version again
- *(deps)* Bump bitflags from 2.4.0 to 2.4.1
- *(deps)* Use precise version for `bitflags`
- *(deps)* Bump thiserror from 1.0.49 to 1.0.50
- *(deps)* Bump taiki-e/install-action from 2.20.3 to 2.20.10
- *(deps)* Bump actions/checkout from 4.1.0 to 4.1.1
- *(deps)* Bump libc from 0.2.148 to 0.2.149
- *(deps)* Bump github/codeql-action from 2.21.9 to 2.22.3
- *(deps)* Bump actions/create-github-app-token from 1.3.0 to 1.5.0
- *(deps)* Bump taiki-e/install-action from 2.19.4 to 2.20.3
- *(deps)* Bump ossf/scorecard-action from 2.2.0 to 2.3.0
- *(deps)* Bump actions/create-github-app-token from 1.2.2 to 1.3.0
- *(deps)* Bump taiki-e/install-action from 2.19.2 to 2.19.4
- *(deps)* Bump step-security/harden-runner from 2.5.1 to 2.6.0

## [0.16.2](https://github.com/robo9k/rust-magic/compare/v0.16.1...v0.16.2) - 2023-10-05

### Added
- Implement `TryFrom` for a few more database path types
- Allow recovering cookie from state transition errors

## [0.16.1](https://github.com/robo9k/rust-magic/compare/v0.16.0...v0.16.1) - 2023-10-03

### Other
- Add a whole lot of rustdoc

## [0.16.0](https://github.com/robo9k/rust-magic/compare/v0.15.1...v0.16.0) - 2023-09-30

### Changed
- [**breaking**] Move everything `Cookie` into mod `cookie`
- [**breaking**] Use typestate for opened/loaded `Cookie`
- [**breaking**] Introduce `DatabasePaths` and `InvalidDatabasePathError`
- [**breaking**] Introduce `OpenError` for `Cookie::open`
- [**breaking**] Introduce `SetFlagsError` for `Cookie::set_flags`
- [**breaking**] Replace `MagicError` with `cookie::Error`
- [**breaking**] Replace internal `ApiViolation` error for `libmagic` with `panic!`

### Other
- Split project/crate README
- *(deps)* Bump thiserror from 1.0.40 to 1.0.49
- *(deps)* Bump libc from 0.2.141 to 0.2.148

## [0.15.1](https://github.com/robo9k/rust-magic/compare/v0.15.0...v0.15.1) - 2023-09-28

### Other
- Fix clippy findings in rustdoc doctests
- Fix clippy findings in tests

## [0.15.0](https://github.com/robo9k/rust-magic/compare/v0.14.0...v0.15.0) - 2023-09-26

### Changed
- *(deps)* [**breaking**] Replace `errno` with `std::io::Error`
- *(deps)* [**breaking**] Upgrade to `bitflags` v2

## 0.14.0 - 2023-09-16

TBD

## 0.13.0 - 2022-08-18

TBD

## 0.13.0-alpha.3 - 2021-11-07

TBD

## v0.13.0-alpha.2 - 2021-11-06

TBD

## 0.13.0-alpha.1 - 2021-10-26

TBD

## 0.12.2 - 2017-04-12

TBD

## 0.12.1 - 2017-04-11

TBD

## 0.12.0 - 2016-10-18

TBD

## 0.11.0 - 2016-07-01

TBD

## 0.10.0 - 2016-06-11

TBD

## 0.9.0 - 2016-01-30

TBD

## 0.8.0 - 2015-07-11

TBD

## 0.7.0 - 2015-02-03

TBD

## 0.6.4 - 2015-01-24

TBD

## 0.6.3 - 2015-01-19

TBD

## 0.6.2 - 2015-01-09

TBD

## 0.6.1 - 2015-01-05

TBD

## 0.6.0 - 2014-12-22

TBD

## 0.5.2 - 2014-12-21

TBD

## 0.5.1 - 2014-12-21

TBD

## 0.5.0 - 2014-12-21

TBD

## 0.4.0 - 2014-12-21

TBD

## 0.3.0 - 2014-12-19

TBD

## 0.2.0 - 2014-11-20

TBD

## 0.1.0 - 2014-11-20

TBD

